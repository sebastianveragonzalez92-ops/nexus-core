...
import {
  ...
  Shield, Zap, Cpu, FileCheck, Award, AlertTriangle
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-blue-950 to-slate-950 text-white">
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20 text-center">
        ...
        <motion.p {...fadeUp(0.2)} className="text-lg text-slate-300 mb-10 max-w-3xl mx-auto leading-relaxed">
          Nexus unifica <span className="text-cyan-300 font-semibold">mantenimiento predictivo</span>, capacitación con{' '}
          <span className="text-cyan-300 font-semibold">certificados válidos</span> y checklists digitales{' '}
          <span className="text-cyan-300 font-semibold">auditables</span> en una sola plataforma.
          Diseñada para minería, metalurgia y manufactura pesada.
        </motion.p>
        ...
        <motion.div {...fadeUp(0.3)} className="flex flex-col sm:flex-row gap-4 justify-center">
          ...
          <Button variant="outline" className="h-12 px-8 text-base border-slate-600 text-slate-200 hover:bg-slate-800 hover:border-slate-400 gap-2" onClick={() => scrollTo('features')}>
      ...
      <section id="roles" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            const border = { cyan: 'border-cyan-500/40 bg-cyan-500/5', blue: 'border-blue-500/40 bg-blue-500/5', emerald: 'border-emerald-500/40 bg-emerald-500/5', purple: 'border-purple-500/40 bg-purple-500/5' }[item.color];
            const iconColor = { cyan: 'text-cyan-400', blue: 'text-blue-400', emerald: 'text-emerald-400', purple: 'text-purple-400' }[item.color];
            const dot = { cyan: 'bg-cyan-400', blue: 'bg-blue-400', emerald: 'bg-emerald-400', purple: 'bg-purple-400' }[item.color];
            const Icon = item.icon;
            return (
              <motion.div key={i} {...fadeUp(i * 0.1)} className={`border ${border} rounded-2xl p-6 flex flex-col gap-4`}>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr>
                <th className="text-left pb-4 w-1/2 pr-4">
                  <div className="flex items-center gap-2 bg-red-950/30 border border-red-500/20 rounded-xl px-4 py-2.5 text-red-400 font-bold text-base">
                    <X className="w-4 h-4" /> Sin Nexus
                  </div>
                </th>
                <th className="text-left pb-4 w-1/2 pl-4">
                  <div className="flex items-center gap-2 bg-emerald-950/30 border border-emerald-500/20 rounded-xl px-4 py-2.5 text-emerald-400 font-bold text-base">
                ...
                <tr key={i} className="border-b border-white/5">
                  <td className="py-3 pr-4">
                    <div className="flex items-start gap-2 text-slate-400">
                      <X className="w-3.5 h-3.5 text-red-400 shrink-0 mt-0.5" />{bad}
                    </div>
                  </td>
                  <td className="py-3 pl-4">
                    <div className="flex items-start gap-2 text-slate-300">
                      <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0 mt-0.5" />{good}
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <section id="features" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            ...
            { icon: FileCheck, title: 'Checklists inteligentes y auditables', desc: 'Plantillas personalizables con validación de IA, fotos, GPS y firma digital desde el celular. Respaldo en la nube con tracking de cambios.' },
            { icon: Award, title: 'Capacitación con certificados válidos', desc: 'Cursos, evaluaciones y certificaciones automáticas para tus técnicos. Cada certificado es descargable, firmado digitalmente y con código de verificación.' },
            { icon: BarChart3, title: 'KPIs y reportes en tiempo real', desc: 'Dashboards automáticos con MTBF, disponibilidad, costos, cumplimiento de mantenciones y estado de certificaciones por técnico.' },
            { icon: Shield, title: 'Cumplimiento normativo garantizado', desc: 'Todo lo que necesitas para auditorías de Sernageomin (Chile), Osinergmin (Perú) y otras entidades. Historial inalterable, certificaciones trazables.' },
            { icon: Zap, title: 'Órdenes de trabajo digitales', desc: 'Crea, asigna y aprueba OTs con flujo completo, notificaciones automáticas y trazabilidad total desde cualquier dispositivo.' },
            ...
            { icon: AlertTriangle, title: 'Conectividad IoT y sensores', desc: 'Integramos con Modbus, OPC UA, SCADA y APIs abiertas. Si tu equipo ya tiene sensores, los conectamos sin fricción.' },
            { icon: Shield, title: 'Seguridad y roles por operación', desc: 'Permisos granulares por rol (admin, supervisor, técnico), auditoría de actividad y datos cifrados en la nube con backups automáticos.' },
          ].map((f, i) => {
            const Icon = f.icon;
            return (
      ...
      <section id="testimonials" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        <h2 className="text-4xl font-bold text-center mb-3">Lo que dicen quienes ya dejaron los paros inesperados</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {[
            ...
            {
              quote: 'Los checklists digitales nos salvaron en una auditoría. El fiscal pidió ver las inspecciones de los últimos 6 meses y en 2 minutos teníamos todo: fotos, fechas, firmas.',
              name: 'Roberto Méndez',
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        <div className="flex items-center justify-center mb-6">
          <span className="bg-yellow-500/20 border border-yellow-500/30 text-yellow-300 text-sm font-semibold rounded-full px-5 py-1.5">
            Promo lanzamiento: 20% OFF en plan anual + onboarding gratis — Válido hasta el {promoDateStr}
          </span>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 items-start">
            ...
            <ul className="space-y-2 text-sm text-slate-200 flex-1">
              {[
                ...
                <li key={f} className="flex items-start gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 shrink-0 mt-1.5" />{f}
                </li>
              ))}
            </ul>
          ...
          <div className="border border-purple-500/50 rounded-2xl p-6 bg-purple-900/20 backdrop-blur flex flex-col gap-4">
            ...
            <ul className="space-y-2 text-sm text-slate-300 flex-1">
              {[
                ...
                'Integración con sensores IoT (Modbus, OPC UA)',
              ].map(f => (
          ...
          <div className="relative border border-yellow-500/50 rounded-2xl p-6 bg-gradient-to-b from-yellow-900/20 to-orange-900/10 backdrop-blur flex flex-col gap-4">
            <div className="absolute -top-3.5 right-6">
              <span className="bg-yellow-500 text-black text-xs font-bold px-3 py-1 rounded-full">Plan Minería</span>
            </div>
            <div>
      ...
      <section id="faq" className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-24">
        ...
        <div className="space-y-4">
          {[
            ...
            { q: '¿Los certificados de capacitación son válidos para Sernageomin?', a: 'Sí. Generamos certificados descargables con código de verificación, tracking de horas y contenido del curso. Personalizables según tus requisitos normativos. Varios clientes los han presentado en fiscalizaciones sin observaciones.' },
